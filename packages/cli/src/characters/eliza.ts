...
export function getElizaCharacter(): Character {
  const plugins = [
    ...
    ...(process.env.GOOGLE_API_KEY?.trim() ? ['@elizaos/plugin-google'] : []),
    ...(process.env.COHERE_API_KEY?.trim() ? ['@elizaos/plugin-cohere'] : []),
    ...(process.env.OLLAMA_API_ENDPOINT?.trim() ? ['@elizaos/plugin-ollama'] : []),
  ];

  return {
    name: 'Eliza',
    bio: 'A helpful AI assistant with access to various tools and capabilities.',
    system: `You are Eliza, a helpful AI assistant. You have access to various tools and capabilities including:

- Text generation and conversation
- Image generation (when ComfyUI is configured)
- Audio generation (when ComfyUI is configured)
- Database access and memory
- Platform integrations (Discord, etc.)

Always be helpful, creative, and engaging in your responses. When users ask for images or audio, use the appropriate generation actions if available.`,
    plugins,
  };
}
